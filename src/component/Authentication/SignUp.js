...
export default class SignUp extends Component {
    ...
    upload = async () => {
        ...
        console.log(fd);
        // let response = await axios.post('http://192.168.225.135:8081/api/v1/admin/createcategory', fd)

        // let response = await axios({
        //     url: "http://192.168.225.135:8081/api/v1/admin/createcategory",
        //     method: 'POST',
        //     data: fd,
        //     headers: {
        ...
        let response = await axios({
            url: "http://192.168.225.135:8081/api/v1/account/signup",
            method: 'POST',
            data: fd,
            headers: {
                Accept: 'application/json',
    ...
    render() {
        ...
        return (
            <View>
                ...
                <View style={{ height: 35, backgroundColor: '#fff', margin: 5 }}>
                    <TextInput placeholder='phone' value={phone} onChangeText={text => this.setState({ phone: text })}></TextInput>
                </View>
                <View style={{ height: 35, backgroundColor: '#fff', margin: 5 }}>
                    <TextInput placeholder='password' value={password} onChangeText={text => this.setState({ password: text })}></TextInput>
                </View>
                <View style={{ height: 35, backgroundColor: '#fff', margin: 5 }}>
                    <TextInput placeholder='name' value={name} onChangeText={text => this.setState({ name: text })}></TextInput>
                </View>
                <View style={{ height: 35, backgroundColor: '#fff', margin: 5 }}>
                    <TextInput placeholder='address' value={address} onChangeText={text => this.setState({ address: text })}></TextInput>
                </View>
                ...
                {/* </View> */}
                {/* <Image source={{ uri: `http://192.168.225.135:8081/image/logo-1677728192587.jpg` }} style={{ width: 200, height: 200 }} /> */}
