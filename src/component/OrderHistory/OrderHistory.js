...
class OrderHistory extends Component {
  ...
  handleDanhSachDonHang = async (status) => {
    ...
    let order = await axios.get(
      `http://192.168.63.6:8081/api/v1/account/donhangtheotaikhoan/${data.userInfo.id_account}`
    );
    // console.log(order.data);
    let res = await axios.get(
      `http://192.168.63.6:8081/api/v1/account/lichsudathang/${data.userInfo.id_account}/${status}`
    );
  ...
  handleHuyDon = async (id_order) => {
    let res = await axios.post(
      `http://192.168.63.6:8081/api/v1/admin/huydonhang/${id_order}`
    );
