...
const signIn = async (email, password) => {
  let res = await axios.post("http://192.168.1.10:8081/api/v1/login", {
    email,
    password,
  });
...
const checkToken = async (token) => {
  ...
  let response = await axios.get(
    "http://192.168.1.10:8081/api/v1/account/info"
  );
...
const addCart = async (token, id_product, quantity, size) => {
  ...
  let response = await axios.post(
    `http://192.168.1.10:8081/api/v1/product/${id_product}`,
    { quantity, size }
  );
...
const getProduct = async () => {
  let response = await axios.get(
    "http://192.168.1.10:8081/api/v1/admin/product?id=ALL"
  );
  return response.data;
};
module.exports = {
